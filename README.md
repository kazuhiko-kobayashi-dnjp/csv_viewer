...
| 操作 | 動作 |
|------|------|
| **ヘッダを右クリック** | 「X軸に設定」「Y軸に追加」「Y軸をリセット」でグラフ軸を指定。ツールバーに現在の設定を表示 |
| **「グラフ」ボタン** | 指定済みのX/Y軸でグラフを即描画。Y軸分離モードあり |
...
1. **ヘッダを右クリック** → 「Y軸に追加」で Y軸列を指定（複数可）
2. 必要なら同じく「X軸に設定」で X軸列を指定（省略すると行番号）
3. ツールバーに現在の `X: ... / Y: ...` 設定が表示される
4. **「グラフ」ボタン**を押すと即座に描画

- **Y軸を分離** チェックで各系列を独立したサブプロットに分割表示
- 巨大ファイルは最大50,000行を等間隔サンプリングして表示（件数はグラフウィンドウ内に表示）
- グラフウィンドウ下部のツールバーでズーム・パン・PNG保存が可能
